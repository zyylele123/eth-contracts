...
contract DOSProxy {
    ...
    event LogDuplicatePubKey(uint x1, uint x2, uint y1, uint y2);
    event LogAddressNotFound(uint x1, uint x2, uint y1, uint y2);
    ...
    function setPublicKey(uint x1, uint x2, uint y1, uint y2)
        ...
        onlyWhitelisted
    {
        BN256.G2Point memory newPubKey = BN256.G2Point([x1, x2], [y1, y2]);
        bytes32 newPubKeyId = keccak256(abi.encodePacked(x1, x2, y1, y2));
                    ...
                    if (pendingGroup[i].pubKeyCounts[newPubKeyId] > groupSize / 2) {
                        ...
                        for (uint l = 0; l < workingGroup.length; l++) {
                            if (BN256.G2Equal(workingGroup[l].finPubKey, newPubKey)) {
                                emit LogDuplicatePubKey(x1, x2, y1, y2);
                                return;
                            }
                        }
        ...
        emit LogAddressNotFound(x1, x2, y1, y2);
    }
